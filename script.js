...
// Curseur custom - APRES le chargement du DOM
window.addEventListener('load', () => {
    ...
    document.querySelectorAll('button, a, input, .lang-btn').forEach(addHover);

    // Observer pour les beat cards dynamiques
    const cardHoverObserver = new MutationObserver(() => {
        document.querySelectorAll('.beat-card:not([data-hover])').forEach(card => {
            card.setAttribute('data-hover', '1');
            addHover(card);
        });
    });
    cardHoverObserver.observe(document.getElementById('beatsGrid') || document.body, { childList: true, subtree: true });
    document.querySelectorAll('.beat-card').forEach(card => { card.setAttribute('data-hover','1'); addHover(card); });
});
...
// Render avec waveform + genre badge
